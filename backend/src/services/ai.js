...
export async function synthesizeMergedNewsFromSources(payload, options = {}) {
  ...
  const prompt = `Ты опытный редактор регионального медиа. Ниже JSON-массив «sources»: предположительно несколько версий одной темы из разных СМИ (поля sourceName, url, title, summary, body и т.д.). Но в массив могли попасть и нерелевантные материалы — про другие события/дела/персонажей.

Шаг 0 — фильтрация (выполни перед всем остальным):
- Определи основной эпизод по большинству источников (ключевые сущности: даты, имена, локации, номера дел, конкретные действия).
- Сформируй внутренний список РЕЛЕВАНТНЫХ источников: те, что действительно описывают именно этот эпизод.
- Все остальные источники ИГНОРИРУЙ полностью: не упоминай их в тексте, не добавляй в блок «Источники», не используй их факты в confirmedFacts/differences/summary. Не пиши фраз вроде «часть источников относится к другим делам» — просто молча отбрось.
- Если после фильтрации остался ровно один релевантный источник — это нормально, сделай аккуратный пересказ только по нему. Если не осталось ни одного — верни JSON с пустым body="" и пояснением в поле notes (см. ниже).
...
3) Блок «Где источники расходятся»: <p><strong>Где источники расходятся</strong></p>, затем 1–2 абзаца <p>. Если расхождений нет — один абзац: нейтрально, что существенных расхождений в сообщениях не видно. Если релевантный источник всего один — этот блок пропусти полностью.
4) Блок «Источники»: <p><strong>Источники</strong></p><ul><li>...</li></ul> — только РЕЛЕВАНТНЫЕ источники (по полю url), с <a href="полный_URL" rel="noopener noreferrer" target="_blank">название</a>. Нерелевантные источники НЕ включай.

В body обязательно вставь осмысленные ссылки <a href="полный_URL" rel="noopener noreferrer" target="_blank">...</a> на релевантные источники (не на все из входа).

Дополнительно в ответе JSON верни массив usedSourceUrls — список url только тех источников, которые ты признал релевантными и фактически использовал в тексте.
...
{"title":"...","body":"...","summary":"...","confirmedFacts":["..."],"differences":["..."],"contentClass":"NEWS","usedSourceUrls":["https://..."],"notes":""}`;
  ...
  const usedSourceUrls = Array.isArray(parsed.usedSourceUrls)
    ? parsed.usedSourceUrls.filter((x) => typeof x === 'string' && /^https?:\/\//i.test(x))
    : [];
  const notes = typeof parsed.notes === 'string' ? parsed.notes.trim() : '';
  return { title, body, summary, confirmedFacts, differences, contentClass, usedSourceUrls, notes };
}
