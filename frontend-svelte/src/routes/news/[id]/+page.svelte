<script lang="ts">
  ...
  // DOMPurify: run client-side for extra XSS protection; re-runs on article change
  ...
  const canonicalUrl = $derived(`https://ivanovo.online/news/${article.id}`);
...
<div class="w-full 2xl:px-6 overflow-x-hidden">
  ...
  <div class="2xl:grid 2xl:grid-cols-[minmax(0,1fr)_48rem_minmax(0,1fr)] 2xl:gap-6 2xl:items-start">
    ...
    <aside class="hidden 2xl:block">
      {#if article.prev}
        <a href="/news/{article.prev.id}" onclick={() => { document.documentElement.dataset.navDir = 'prev'; }} class="block bg-white p-6 md:p-8 rounded-[2rem] border border-gray-100 shadow-sm opacity-60 hover:opacity-80 transition-opacity">
          <div class="mb-6">
    ...
    <aside class="hidden 2xl:block">
      {#if article.next}
        <a href="/news/{article.next.id}" onclick={() => { document.documentElement.dataset.navDir = 'next'; }} class="block bg-white p-6 md:p-8 rounded-[2rem] border border-gray-100 shadow-sm opacity-60 hover:opacity-80 transition-opacity">
          <div class="mb-6">
