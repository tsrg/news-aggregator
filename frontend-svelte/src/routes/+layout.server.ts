...
export const load: LayoutServerLoad = async ({ fetch }) => {
  ...
  const [sectionsRes, headerRes, footerRes] = await Promise.allSettled([
    fetch(`${serverBase}/api/sections`).then((r) => r.json() as Promise<Section[]>),
    fetch(`${serverBase}/api/menus/header`).then((r) => r.json() as Promise<{ items?: MenuItem[] }>),
    fetch(`${serverBase}/api/menus/footer`).then((r) => r.json() as Promise<{ items?: MenuItem[] }>),
  ]);

  const sections: Section[] = sectionsRes.status === 'fulfilled' ? sectionsRes.value : [];
