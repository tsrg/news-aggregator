    ...
    <div class="space-y-6">
      <div class="border-b border-gray-100 pb-6">
        ...
        <div class="max-w-2xl space-y-6">
          ...
          <!-- Tavily (доп. источники для обзора события) -->
          <div class="rounded-xl border border-indigo-100 bg-indigo-50/40 p-4 space-y-2">
            <div>
              <label class="block text-sm font-medium text-gray-800 mb-1">Tavily API Key</label>
              <p class="text-xs text-gray-600 mb-2">
                Необязательно. При «Сформировать обзор события» подмешиваются краткие выдержки из веба (дополнительно к похожим материалам в базе).
              </p>
            </div>
            <div class="relative">
              <input
                ...
                autocomplete="off"
                class="w-full bg-white border border-gray-200 rounded-xl p-3.5 text-gray-900 focus:bg-white focus:border-indigo-400 focus:ring-2 focus:ring-indigo-100 focus:outline-none transition-all pr-12"
                placeholder="tvly-..."
              />
              <button
                type="button"
              ...
              >
                <svg v-if="showTavilyApiKey" xmlns="http://www.w3.org/2000/svg" class="h-5 w-5" viewBox="0 0 20 20" fill="currentColor">
                  <path d="M10 12a2 2 0 100-4 2 2 0 000 4z" />
                  <path fill-rule="evenodd" d="M.458 10C1.732 5.943 5.522 3 10 3s8.268 2.943 9.542 7c-1.274 4.057-5.064 7-9.542 7S1.732 14.057.458 10zM14 10a4 4 0 11-8 0 4 4 0 018 0z" clip-rule="evenodd" />
                </svg>
                <svg v-else xmlns="http://www.w3.org/2000/svg" class="h-5 w-5" viewBox="0 0 20 20" fill="currentColor">
            ...
            <p class="text-xs text-gray-500">Очистите поле и сохраните, чтобы сбросить ключ (будет использоваться только переменная окружения, если задана).</p>
          </div>
...
interface AISettings {
  ...
  temperature: number;
  maxTokens: number;
  /** Tavily Search — веб-источники для обзора события */
  tavilyApiKey: string;
}
